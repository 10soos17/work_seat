...
import random
import csv
#mport fourth_mine_kv as tmk

def readSeat(classNum):

    savedClassList=[]

    seatReadFile = open('./fourth_seatData.csv','r',encoding='UTF-8')
...
def randomSelectSeat(classNum, name, seatLimit):
    ...
    if seatLimit - len(savedNumList) == 0:
        ...
        if savedNumList.count(seatNum) == 0:
            seatAppendFile = open('./fourth_seatData.csv','a',encoding='UTF-8')
            #wr =csv.writer(seatAppendFile)
        #    wr.writerow([classNum, seatNum, name])
...
def randomDeleteSeat(classNum, name):
    ...
    if len(tempList) == count:
        ...
        seatDelFile = open('./fourth_seatData.csv','w',encoding='UTF-8')
